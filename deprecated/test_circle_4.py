...
class BigMixer(QWidget):
    ...
    def paintEvent(self, event):
        ...
        scale_height = int((self.height() - buffer - 50) * self.level / 100)
        scale_rect = QRect(10, buffer + self.height() - scale_height - 10, self.width() - 70, scale_height)
        ...
        tick_length = 5
        tick_spacing = (self.height() - buffer - 40) / 100
        degree_spacing = 5
        for level in range(0, 101, degree_spacing):
            y = buffer + self.height() - 30 - int((level / 100) * (self.height() - buffer - 40))  # Adjusted y coordinate calculation

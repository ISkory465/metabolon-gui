...
class BigMixer(QWidget):
    def __init__(self, level=10):
        super().__init__()
        self.setMinimumSize(100, 300)  # Increased the minimum height to accommodate the circle and existing elements
        self.level = level
    ...
    def paintEvent(self, event):
        ...
        circle_radius = 30
        circle_center = QPoint(self.width() // 2, -circle_radius)  # Position the center above the widget
        painter.drawEllipse(circle_center, circle_radius, circle_radius)

        # Draw the infinity sign
        ellipse1 = QRectF(60, 400, 100, 30)
        ellipse2 = QRectF(140, 400, 100, 30)
